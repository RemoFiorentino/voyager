	...
	if(document.getElementById('filemanager')){
		...
		var VoyagerMedia = function(o){
			...
			this.init = function(){
		        ...
		        // Original init.
		        // getFiles('/');
		        // new Init to get folders from url.
		        initFromUrl();
				...
				/*
				*	Function to init the media manager, considering that the url may contain a variable
				*	with the folders path to initialize instead of root. In case that a folder does not exist
				*	or the last one is empty.
				*/
				function initFromUrl() {
		          var arrayfolders = readUrlParameters("folders");
					if (arrayfolders.length > 0 ){
						folderExist(arrayfolders);
					} else {
						getFiles('/');
					}
				}
				...
				/*
				*	Function to create the ajax request to get the files of that are stored in path. 
				*/
		        function getFilesByAjax(path){
		            return $.ajax({
		              type: 'POST',
		              url: options.baseUrl + '/media/files',
		              data: {
		                folder: path,
		                _token: CSRF_TOKEN
		              }
		        ...
		        /*
		        *	Function that collect all the ajax request by all the folders got from the url.
		        */
				function folderExist(folders) {
					var path = '/';
		          	var ajaxCalls = []
					$.each(folders, function (index, folder) {
						path = path + folder + "/";
		            	ajaxCalls.push(getFilesByAjax(path))
					});
		          	var existance = true
				...
				//Function to get the array of folders from the url.
				function readUrlParameters(name) {
					var UrlTarget = new URL(window.location.href);
					var params = UrlTarget.searchParams.get(name);
					params = params == null ? "" : params;
					var arrayfolders = params.split(",");
					return arrayfolders;
				}
